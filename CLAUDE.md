...
- **The `:baselineprofile` module generates the app's startup profile** and is inert during ordinary builds. Run `./gradlew :app:generateReleaseBaselineProfile` with a device or a *rooted* (AOSP, non-Play) emulator attached; it writes `app/src/release/generated/baselineProfiles/`, which release builds then pick up. Until it has been run the app ships with only the profiles the Compose libraries carry, and none for its own code. The plugin needs `androidx.baselineprofile` 1.5.x — 1.4.1 rejects AGP 9's modules outright — and `com.android.test` has to be declared `apply false` in the root build file, since a version cannot be resolved for a plugin already on the classpath.
