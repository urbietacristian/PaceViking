...
- **There is no `material-icons-extended` dependency.** The app draws six icons; four come from `material-icons-core` and the two that do not (`ContentCopy`, `DragHandle`) are declared in `ui/WorkoutIcons.kt` as extension properties on `Icons.Filled`, built from the library's own path data with its own `materialIcon`/`materialPath` builders. R8 already stripped the unused thousands from release, so this does not change the shipped APK — it halves the debug one (23.2 MB → 12.4 MB), which is what every incremental install pushes to the device. Adding an icon the core set lacks means adding it to that file, not restoring the dependency.
- **The `:baselineprofile` module generates the app's startup profile**, and doing so takes two steps because of an AGP 9 interaction. The plugin derives its `nonMinifiedRelease` variant from `release`, which here carries `optimization { enable = true }` — so R8 runs on the variant that is supposed to stay un-obfuscated, and the profile gets recorded against *obfuscated* names. Those look fine in the file, but `compileReleaseArtProfile` applies the release mapping to translate real names into obfuscated ones, finds nothing to translate, and silently drops them: measured, that left 0 rules for `MainActivityKt` and ~9 in total for app code. Setting `optimization.enable = false` on `nonMinifiedRelease` (via `buildTypes.all`) does not stick — the plugin overwrites it. So to regenerate: flip `release`'s `optimization.enable` to `false`, run `./gradlew :app:generateReleaseBaselineProfile` with a device or a *rooted* (AOSP, non-Play) emulator attached, then flip it back to `true`. That yields real names (15k rules, 161 for `MainActivityKt`), and the mapping is applied properly on the next release build — 134 app rules survive into `assets/dexopt/baseline.prof`, the rest being methods R8 inlines away. Verify a regeneration by grepping the generated file for `MainActivityKt`: if it is absent, R8 ran and the profile is worthless. The profile lives in `app/src/release/generated/baselineProfiles/` and is committed, so ordinary release builds need no device. **It also wipes the app's data on the device it runs against**: Macrobenchmark installs its own variant over whatever is there and uninstalls it when the run ends, taking the Room database with it. Back the database up first — the app is debuggable, so `adb exec-out run-as com.example.paceviking cat databases/workout_database` works without root, and all three files (`workout_database`, `-shm`, `-wal`) have to be copied: the WAL holds everything not yet checkpointed into the main file. Restoring is the same three files piped back through `run-as` with the app force-stopped, after an `installDebug` to get a debuggable build on the device again.
